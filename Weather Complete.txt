...
to be the deafault output the user does not specifiy the amount of days#>
[int] $Days = 1,
[char] $CUnit
)
...
if( $CUnit -eq 'C' -or $CUnit -eq 'c')
{
<#created a for loop in order print on the amount of days needed to be displayed#>
for($i = 0; $i -lt $Days; $i++)
{
<#created a hash table to make all my information objects in order to print them out in list or table formula.
Used ordered to allow the information 
...
elseif($CUnit -eq 'B' -or $CUnit -eq 'b')
{
for($i = 0; $i -lt $Days; $i++)
{
<#created a hash table to make all my information objects in order to print them out in list or table formula.
